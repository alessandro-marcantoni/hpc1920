...
void convex_hull(const points_t *pset, points_t *hull)
{
    ...
    do {
        ...
        next = (cur + 1) % n;
#pragma omp parallel for default(none) shared(hull, p, leftmost, cur, local_next) private(i, j) firstprivate(next)
        for (j=0; j<n; j++) {
            if (LEFT == turn(p[cur], p[next], p[j])) {
                next = j;
            }
            local_next[omp_get_thread_num()] = next;
        }
        for (j=0; j<omp_get_max_threads(); j++) {
			if (LEFT == turn(p[cur], p[next], p[local_next[j]])) {
				next = local_next[j];
			}
		}
        assert(cur != next);
